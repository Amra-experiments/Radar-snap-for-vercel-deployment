<template>
  <div class="project-switcher">
    <Dropdown v-model="selectedProject" :options="projects" optionLabel="name" optionValue="id"
      placeholder="Select Project" :loading="loading" @change="handleProjectChange" class="w-48"
      ...
      <template #option="slotProps">
        <div class="flex items-center justify-between w-full py-2">
          ...
          <Badge :value="slotProps.option.role" :severity="getRoleSeverity(slotProps.option.role)" />
        </div>
      </template>

      <template #footer>
        <div class="border-t border-gray-200 pt-2">
          <Button @click="handleCreateProject" text class="w-full justify-start">
